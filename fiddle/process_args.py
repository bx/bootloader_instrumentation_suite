    ...
    def __init__(self, name="", plugin=None,
        ...
        class SubstageFileAction(argparse.Action):
            def __init__(self, option_strings, dest, **kwargs):
                ...
                super(SubstageFileAction, self).__init__(option_strings,
                                                         dest, **kwargs)
            ...
            def __call__(self, parser, namespace, values, option_string=None):
                ...
                if stagename not in self.stagenames:
                    raise argparse.ArgumentError(self,
                                                 "%s not a valid stage, must "
                                                 "be one of %s" %
                                                 (stagename,
                                                  str(self.stagenames)))
                if not os.path.exists(f):
                    raise argparse.ArgumentError("Substage definition file I "
                                                 "am trying to import from "
                                                 "'%s' not found" % f)
                if not os.path.exists(d):
                    raise argparse.ArgumentError("Region definition file I "
                                                 "am trying to import from "
                                                 "'%s' not found" % d)
        ...
        class SubstageNameAction(argparse.Action):
            def __init__(self, option_strings, dest, **kwargs):
                ...
                super(SubstageNameAction, self).__init__(option_strings,
                                                         dest, **kwargs)
        ...
        if plugin is not None:
            ...
            cmds.add_argument('-c', '--create',
                              help='Create new result directory for '
                              'current instance of '
            ...
            cmds.add_argument('-L', '--list_policies', default=False,
                              action="store_true",
            ...
            cmds.add_argument('-l', '--list_test_runs', action="store_true",
                              default=False)
            cmds.add_argument('-p', '--postprocess_trace', default=[],
                              action="append",
                              choices=manager.PostTraceLoader.supported_types,
                              help="Run trace postprocessing command")
            cmds.add_argument('-P', '--print_trace_commands',
                              action='store_true',
        ...
        parser.add_argument("-T", "--trace_methods",  action="append",
                            choices=[m.name for m in
                                     Main.object_config_lookup("TraceMethod")])
        parser.add_argument('-s', '--stages', action='append', default=[])
        parser.add_argument('-n', '--select_policy',
                            action=SubstageNameAction, nargs=2)
        parser.add_argument('-H', '--host', action="store", default=host.name,
                            choices=[m.name for m in
                                     Main.object_config_lookup("HostConfig")])
